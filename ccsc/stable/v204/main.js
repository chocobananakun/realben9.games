...
Game.Launch=function()
{
	...
	Game.Init=function()
	{
		...
		Game.LoadSave=function(data)
		{
			...
			else
			{
				if (App)
				{
					App.getMostRecentSave(function(data){Game.LoadSave(data);});
					return false;
				}
				if (Game.useLocalStorage)
				{
		...
		Game.crate=function(me,context,forceClickStr,id)
		{
			...
			return '<button aria-labelledby="ariaReader-'+me.type+'-'+me.id+'"'+
		...
		Game.ClickCookie=function(e,amount)
		{
			...
			if (Game.OnAscend || Game.AscendTimer>0 || Game.T<3 || now-Game.lastClick<1000/(e.detail===0?3:50)) {}
			else
			{
				...
				str+='<div class="framed" style="margin:4px 48px;">'+
					...
					'<div class="block" style="padding:0px;margin:8px 4px;">'+
						...
						'<div class="title">'+loc("General")+'</div>'+
							'<div class="listing" style="box-shadow:0px 0px 0px 1px rgba(255,255,255,0.2);background:rgba(0,0,0,0.5);"><div style="padding:2px 2px;opacity:0.75;font-size:12px;float:left;" class="smallFancyButton">'+loc("Language: %1",'<b>'+Langs[locId].name+'</b>')+'</div><a style="font-size:15px;text-align:center;float:right;width:auto;min-width:130px;" class="option smallFancyButton" '+Game.clickStr+'="PlaySound(\'snd/tick.mp3\');Game.showLangSelection();">'+(!EN?'Change language<div class="line"></div>':'')+loc("Change language")+'</a><div style="clear:both;text-align:right;padding-bottom:2px;"></div></div>'+
				...
				'<div class="framed" style="margin:4px 48px;">'+
					'<div class="block" style="padding:0px;margin:8px 4px;">'+
						...
						'<div class="listing">'+
							...
							(EN?(Game.WriteButton('format','formatButton',loc("Short numbers")+ON,loc("Short numbers")+OFF,'BeautifyAll();Game.RefreshStore();Game.upgradesToRebuild=1;',1)+(EN?'<label>(shorten big numbers)</label>':'')+'<br>'+
							...
							Game.WriteButton('notScary','notScaryButton',loc("Scary stuff")+OFF,loc("Scary stuff")+ON)+'<br>'+
							Game.WriteButton('timeout','timeoutButton',loc("Sleep mode timeout")+OFF,loc("Sleep mode timeout")+ON,0,true)+'<label>('+loc("on slower computers, the game will put itself in sleep mode when it's inactive and starts to lag out; offline CpS production kicks in during sleep mode")+')</label><br>'+
						'</div>'+
		...
		Game.getNewTicker=function(manual)//note : "manual" is true if the ticker was clicked, but may also be true on startup etc
		{
			...
			if (Game.windowW<1000) list=[NEWS+(EN?'help!':loc("help me!"))];
		...
		AddEvent(Game.tickerL,'click',function(event){
			Game.Ticker='';
			Game.TickerClicks++;
			if (Game.windowW<1000) {}//{Game.Win('Stifling the press');}
		...
		Game.BuildStore=function()//create the DOM for the store's buildings
		{
			...
			{
				var me=Game.Objects[i];
				str+='<button aria-labelledby="ariaReader-product-'+(me.id)+'" class="product toggledOff" '+Game.getDynamicTooltip('Game.ObjectsById['+me.id+'].tooltip','store')+' id="product'+me.id+'"><div class="icon off" id="productIconOff'+me.id+'" style=""></div><div class="icon" id="productIcon'+me.id+'" style=""></div><div class="content"><div class="lockedTitle">???</div><div class="title productName" id="productName'+me.id+'"></div><span class="priceMult" id="productPriceMult'+me.id+'"></span><span class="price" id="productPrice'+me.id+'"></span><div class="title owned" id="productOwned'+me.id+'"></div><label class="srOnly" style="width:64px;left:-64px;" id="ariaReader-product-'+(me.id)+'"></label></div>'+
		...
		Game.Win=function(what)
		{
			if (typeof what==='string')
			{
				if (Game.Achievements[what])
				{
					...
					{
						var name=Game.Achievements[what].shortName?Game.Achievements[what].shortName:Game.Achievements[what].dname;
						Game.Achievements[what].won=1;
						Game.Notify(loc("Achievement unlocked"),'<div class="title" style="font-size:18px;margin-top:-2px;">'+name+'</div>',Game.Achievements[what].icon);
						if (Game.CountsAsAchievementOwned(Game.Achievements[what].pool)) Game.AchievementsOwned++;
						Game.recalculateGains=1;
						if (App) App.gotAchiev(Game.Achievements[what].id);
					}
				}
			}
					...
					if (me.selected && onWrinkler==0 && Game.CanClick)
					{
						...
						if (Game.Click && Game.lastClickedEl==l('backgroundLeftCanvas'))
						{
							...
							else
							{
								...
								me.hurt=1;
								me.hp-=0.75;
								if (Game.prefs.particles && !Game.WINKLERS && !(me.hp<=0.5 && me.phase>0))
								{
		...
		Game.DrawWrinklers=function()
		{
			...
			if (selected && Game.Has('Eye of the wrinkler'))
			{
				...
				for (var i=0;i<Math.floor(dist/12);i++)
				{
					...
					ctx.beginPath();
					ctx.arc(xC,yC,4+2*Math.pow(Math.sin(-Game.T*0.2+i*0.3),4),0,2*Math.PI,false);
					ctx.fill();
					ctx.stroke();
				}
	...
	Game.Logic=function()
	{
		...
		{
			if (Game.toReload) location.reload();
	...
	Game.Draw=function()
	{
		...
		if (!Game.OnAscend)
		{
			...
			if (str.length>11) str=str.replace(' ','<br>');
